  ...
  try {
    ...
    if (authType == "Teacher") {
      ...
      slots = await Slot.find({ teacher }).populate({
        path: 'group',
        populate: {
          path: 'project employee1 employee2 employee3'
        }
      })
...
const editSlot = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    {
      //group membrs
      const {valueEmployee2,valueEmployee3} = req.body.group
      ...
      if(project)
      {
        const group = await Group.create({ employee1:auth, employee2:value2, employee3: valueEmployee3,project: project._id});
        if(group){
